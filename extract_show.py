...
a=gallery_feature
b=np.transpose(query_feature)
# d=np.matmul(a, b)/np.linalg
distmat = torch.FloatTensor(1 - np.matmul(a, b)).cuda() #Cosine distance
...
ax.hist(intra.detach().cpu().numpy(), b, histtype="stepfilled", alpha=0.6, color = 'blue', density=True, label='Intra-person')
ax.hist(inter.detach().cpu().numpy(), b, histtype="stepfilled", alpha=0.6, color = 'green', density=True, label='Inter-person')
...
fig.savefig('AGW.png',dpi=1000,format='png')
plt.show()