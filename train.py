...
if dataset == 'sysu':
    ...
    test_mode = [1, 2]  # thermal to visible   只有红外查可见光的模式
elif dataset == 'regdb':
...
transform_train_list = [
    ...
    transforms.ToTensor(),
    RandomLinear(probability=0.8, min_out=1e-2), #work  之前是0.3   0.7对 regdb 的V2T数据集最好 0.8对T2V最好
...
transforms_gray=transforms.Grayscale(num_output_channels=3)
end = time.time()
if dataset == 'sysu':
    # training set
...
def train(epoch):
    ...
    #     transforms.ToPILImage(),
    #     #transforms.Resize((15, 8)),
    #     transforms.ToTensor(),
    # ])
    # mask_GT=Image.open('66.jpg')
    # mask_GT=np.array(mask_GT)
    # mask_GT=transform_mask(mask_GT)
    # mask_GT=torch.unsqueeze(mask_GT,dim=0)
    # mask_GT=mask_GT.expand(112,-1,-1,-1)



    for batch_idx, (input1, input2, label1, label2,mask_v,mask_c) in enumerate(trainloader):
        #print(input1[0])#[-1,1]  如果不标准化norm 就是【0，1】之间的值

        labels = torch.cat((label1, label2), 0)

        #process of mask-GT
        mask_v=torch.unsqueeze(mask_v,dim=1)
        mask_t=mask_v
        mask=torch.cat((mask_v,mask_t),0) # 112,1,384,192
        mask=mask.float()
        mask=mask/255.0
        # s=mask[0,0,:,:].numpy()
        # np.savetxt('s.txt',s)

        mask_c = torch.unsqueeze(mask_c, dim=1)
        mask_tmp = mask_c
        maskc_g = torch.cat((mask_c, mask_tmp), 0)  # 112,1,384,192
        maskc_g = maskc_g.float()
        maskc_g = maskc_g / 255.0
        ...
        maskc_g = Variable(maskc_g.cuda())

        # input1_g=transforms_gray(input1)
        # input2_g=transforms_gray(input2)
        ...
        feat, output, feat_globe, out_globe,maskc_globe = net(input1, input2)
        ...
        for i in range(args.part_num):
            ...
            cc_feat = feat[i].chunk(2, 0) # on
            loss_cc += args.w_hc * criterion_cc(cc_feat[0], cc_feat[1], label1, label2, epoch)

        loss_globe = criterion_id(out_globe, labels) * 2
        #feat_globe=nn.functional.interpolate(feat_globe,(384,192),mode='bilinear',align_corners=True)
        #print(feat_globe.dtype)
        #print(mask.dtype)


        loss_mask = mse_loss(feat_globe,mask)*10+mse_loss(maskc_globe,maskc_g)*10
        ...
        loss = loss_id + loss_tri + loss_cc + loss_globe+ loss_mask   #这里权重考虑调整
        ...
        #loss_mask.update(loss_mask.item(), 2 * input1.size(0))
        total += labels.size(0)
        ...
        if batch_idx % 50 == 0:
            print('Epoch: [{}][{}/{}] '
                  ...
                  'Accu: {:.2f}'.format(
                epoch, batch_idx, len(trainloader), current_lr, loss_cc, loss_globe,loss_mask,
