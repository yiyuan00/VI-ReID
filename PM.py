...
class PM(nn.Module):
    def __init__(self, feature_dim, blocks=7, num_stripes=6, local_conv_out_channels=256, erase=0, loss={'htri'},
                 avg=False, **kwargs):
        ...
        self.bn0 = nn.BatchNorm1d(256 * blocks)
        ...
        self.pcb2_pool_list, self.pcb2_conv_list, self.pcb2_batchnorm_list, self.pcb2_relu_list = pcb_block(
            self.num_ftrs, 2, local_conv_out_channels, feature_dim, avg)
        self.pcb4_pool_list, self.pcb4_conv_list, self.pcb4_batchnorm_list, self.pcb4_relu_list = pcb_block(
            self.num_ftrs, 4, local_conv_out_channels, feature_dim, avg)
        # self.pcb8_pool_list, self.pcb8_conv_list, self.pcb8_batchnorm_list, self.pcb8_relu_list = pcb_block(
        #     self.num_ftrs, 8, local_conv_out_channels, feature_dim, avg)
    ...
    def forward(self, x):
        ...
        feat_list = spp_vertical(feats, self.pcb2_pool_list, self.pcb2_conv_list, self.pcb2_batchnorm_list,
                                 self.pcb2_relu_list, 2, feat_list)
        feat_list = spp_vertical(feats, self.pcb4_pool_list, self.pcb4_conv_list, self.pcb4_batchnorm_list,
                                 self.pcb4_relu_list, 4, feat_list)
        # feat_list = spp_vertical(feats, self.pcb8_pool_list, self.pcb8_conv_list, self.pcb8_batchnorm_list,
        #                          self.pcb8_relu_list, 8, feat_list)
...
# feat=torch.zeros([56,2048,24,12])
# model=PM(feature_dim=206)
# out=model(feat)
# print(out[0].shape)
